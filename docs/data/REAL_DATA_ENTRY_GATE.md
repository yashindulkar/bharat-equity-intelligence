...
Provider scorecards bind provider/product/product version and policy/agreement snapshot. Every score and weight is a finite `Decimal`; dimensions are exact and unique; weights are within 0–1 and total exactly 1; evidence has an availability time; assessment and evidence times cannot exceed the evaluation cutoff. The proposed weighted total uses deterministic Decimal arithmetic and `ROUND_HALF_EVEN` to two decimal places. No score is calculated until all hard gates pass.

`evaluate_hard_gates()` alone creates a factory-controlled, content-addressed `EvaluationArtifact`. It binds capability, policy, lifecycle-registry/lifecycle, scorecard and request snapshots, cutoff, reasons, score and schema version. Publication revalidates the artifact against supplied immutable inputs. Arbitrary caller data or any changed snapshot is rejected with `EVALUATION_ARTIFACT_INVALID`.
